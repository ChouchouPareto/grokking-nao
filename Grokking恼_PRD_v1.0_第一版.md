# Grokking恼 产品需求文档（PRD）v1.0｜第一版

> 文档状态：第一版草案，可进入原型与技术拆解；尚未最终定稿  
> 更新时间：2026-08-25  
> 产品类型：0→1 创新产品 + AI 能力  
> 当前阶段：产品第一版已形成，等待后续验证与迭代
...
2. 节点进入空间并自动形成初步布局。
3. 用户可拖动、靠近、分离或连接节点。
4. 创建连接后，用户可选择补充一句连接说明；MVP 不强制选择关系类型。
5. 系统根据节点语义与结构调整布局和氛围。

### 9.2.1 3D 空间操作规则

- 【已确认】镜头采用旋转、缩放和平移三类受控操作，不提供游戏式自由行走。
- 【已确认】点击节点进入聚焦状态，突出当前节点及其一层相邻关系，并相对弱化无关节点。
- 【已确认】用户可以一键退出聚焦并返回全局视图。
...
1. 当网络中至少存在 3 个正式关键词节点时，AI 可以显示轻量提示，但不自动弹出或展开内容；用户在任意时点均可主动点击“帮我展开”。
2. 用户点击“帮我展开”，主动请求本轮建议。
3. 每轮最多出现 3 条建议，类型包括新关键词、潜在连接和启发式问题。
4. 每条建议展示简短理由，说明其与当前节点或结构的关系。
5. 新节点和连接以半透明候选态直接出现在空间中，不进入正式网络。
6. 用户接受、修改后接受或拒绝建议；接受后候选内容才转为正式状态。
...
- 本文档已具备进入交互原型和技术方案拆解的基础。
- 当前为 PRD 第一版草案，不等同于最终定稿。
