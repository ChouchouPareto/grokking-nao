# Grokking恼 产品需求文档（PRD）v1.2｜认知宇宙视觉版

> 文档状态：v1.2 已更新，与当前产品实现基线对齐
>
> 更新时间：2026-09-06
>
> 产品类型：0→1 创新产品 + AI 能力
>
> 当前阶段：认知宇宙视觉与关键词渐进披露已合并，进入用户验证
>
> 实现基线：产品 v0.5.0｜GitHub `main` @ `098c4da`
>
...
- 【已确认】3D 默认视觉采用深空“认知宇宙”：概念节点以不同色温的星点呈现，核心概念带有柔光与多层轨道，远近粒子形成全局发散感。
- 【已确认】默认仅常显核心关键词；其他关键词在悬停、选中、聚焦或进入连接状态时浮现，用户可通过顶部“关键词”按钮一键显示或隐藏全部标签。
- 【已确认】标签必须从星点上方浮现，不遮挡节点发光核心；隐藏标签不进入键盘焦点顺序。
...
9. 后续可接入成本更低的小模型持续计算潜在关系，但模型只写入候选连接层，必须可解释、可撤销，并由用户确认后转为正式连接。
...
| FR-VIEW-007 | 认知宇宙视觉 | P0 | 以深空粒子、核心轨道、节点色温和空间雾建立全局发散的 3D 认知空间 |
| FR-VIEW-008 | 关键词渐进披露 | P0 | 核心词常显，其他关键词按悬停、选中和聚焦浮现，并支持一键全部显隐 |
...
| 认知宇宙 | 默认形成深空粒子与核心轨道层次；非核心关键词默认收起，悬停可发现且可一键全部显示；减少动态设置有效 |
...
| 版本类型 | 格式 | 用途 | 当前版本 |
|---|---|---|---|
| PRD 文档版本 | `v主版本.次版本` | 记录产品范围、交互规则和验收口径；需求语义或验收标准变化时升级 | `v1.2` |
| 产品实现版本 | `v主版本.次版本.修订版` | 记录可运行产品里程碑；功能增强升次版本，缺陷修复升修订版 | `v0.5.0` |
...
| v1.2 | 2026-09-05 | 认知宇宙视觉版 | 将思考空间升级为深空认知宇宙，补充核心轨道、分支色温、关键词渐进披露与小模型候选连线原则 |
...
| v0.5.0 | 2026-09-06 | 已合并 | 深空三臂粒子场、核心概念轨道、分支色温、关键词按需浮现与顶部显隐控制；GitHub `main` @ `098c4da` |

## 28. 文档状态

- 本文档已更新至 PRD v1.2，并与产品 v0.5.0 实现基线对齐。
