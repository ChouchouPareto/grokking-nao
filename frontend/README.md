...
- **意图驱动的 AI 建议**：先识别当前意图与思考阶段，再从动态缺失维度生成节点/连接/追问；不使用固定的「成本/客户」模板
- **双击即思考**：双击 3D 画布空白处添加关键词；默认在 3 秒防抖后扫描它与现有节点的潜在联系，也可在工具栏关闭自动扫描
- AI 节点/连接以半透明候选态出现，可接受、编辑后接受或拒绝
- **节点头脑风暴**：选中正式节点后发起联想，3 个半透明候选围绕来源节点出现；点击候选后再决定接受、编辑或拒绝
- **3D 工作台布局**：项目首页直达空间；左侧为视图/绘制工具，右侧为节点上下文、AI 解释和阶段总结
- **Hyperspeed 思路光轨**：画布底层使用 React Bits Hyperspeed，并融合淡紫、雾蓝、暖杏品牌色；不拦截画布操作，移动端与减少动态效果模式自动停用
- **阶段总结**：可总结整个正式图谱或当前聚焦范围，确认后保存在 Idea 历史中
- **邀请体验登录**：邀请码仅在服务端校验，签名会话默认 1 小时；真实邀请码和签名密钥不进入客户端

**暂未实现**（后续切片）：一次性邀请码激活记录、正式账户与付费、JSON 导入导出、撤销重做、导出图片。
...
- 所有项目数据仅保存在浏览器 IndexedDB（数据库 `grokking`），当前邀请码登录只控制访问，不提供云端同步。
- 清除浏览器数据会导致 Idea 丢失；JSON 备份导出在后续切片提供。
- 标题生成为规则化临时方案（取前 18 字），后续可换 LLM。
- AI 请求只发送当前 Idea 的文本/节点/连接、触发节点与已拒绝摘要，不发送其他本地数据。
- IndexedDB `schemaVersion = 3`；旧数据打开时自动补齐意图画像、自动关系扫描配置和总结集合。
